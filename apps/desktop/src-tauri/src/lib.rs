...
fn default_api_mode() -> &'static str {
...
fn api_mode() -> String {
    env_or_default("ROUTA_DESKTOP_API_MODE", default_api_mode())
}
...
fn api_url(api_host: &str, port: u16, api_mode: &str) -> String {
    std::env::var("ROUTA_DESKTOP_API_URL").unwrap_or_else(|_| {
        if cfg!(debug_assertions) && api_mode == "external" && port == 3000 {
            format!("http://{}:{}/?runtime=tauri", api_host, port)
        } else {
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let port = api_port();
            let api_url = api_url(&api_host, port, &api_mode);
