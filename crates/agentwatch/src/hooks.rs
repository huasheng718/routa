...
pub fn try_forward_hook_to_runtime(
    ...
    if let Err(err) = send_runtime_realtime_message(&ctx, &message) {
        append_runtime_feed(&ctx, &message)?;
...
pub fn try_forward_git_event(ctx: &RepoContext, event_name: &str, args: &[String]) -> Result<bool> {
    ...
    match send_runtime_realtime_message(ctx, &message)
    {
        Ok(_) => Ok(true),
        Err(err) => {
            append_runtime_feed(ctx, &message)?;
...
fn send_runtime_realtime_message(ctx: &RepoContext, message: &RuntimeMessage) -> Result<()> {
    ipc::send_socket_message(&ctx.runtime_socket_path, message)
        .or_else(|_| ipc::send_tcp_message(&ctx.runtime_tcp_addr, message))
}

fn append_runtime_feed(ctx: &RepoContext, message: &RuntimeMessage) -> Result<()> {
    ipc::send_message(&ctx.runtime_event_path, message)
}

fn send_runtime_message(ctx: &RepoContext, message: &RuntimeMessage) -> Result<()> {
    send_runtime_realtime_message(ctx, message).or_else(|_| append_runtime_feed(ctx, message))
}
