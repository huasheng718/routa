...
use crate::api::repo_context::get_current_routa_repo_root;
use crate::shell_env;
...
async fn execute_auditor_command(
    ...
    let tool_repo_root = get_current_routa_repo_root()
        .or_else(|| std::env::current_dir().ok())
        .ok_or_else(|| "Failed to resolve Routa tool repository root".to_string())?;

    // Quick pre-check: verify specialist binary exists before attempting execution
    let local_binary_path = tool_repo_root.join("target/debug/routa");
...
fn audit_provider_command(provider: &str) -> Option<&'static str> {
    match provider.trim().to_ascii_lowercase().as_str() {
        "auggie" => Some("auggie"),
        "claude" | "anthropic" => Some("claude"),
        "codex" => Some("codex-acp"),
        "copilot" => Some("github-copilot-acp"),
        "gemini" => Some("gemini"),
        "kimi" => Some("kimi"),
...
pub(crate) async fn run_instruction_audit(
    ...
    if normalized_provider.is_empty() {
        return build_heuristic_audit(
            ...
            to_duration_ms(),
            provider,
            Some("Provider is empty, using heuristic fallback"),
        );
    }

    if let Some(command) = audit_provider_command(provider) {
        if shell_env::which(command).is_none() {
            return build_heuristic_audit(
                source,
                to_duration_ms(),
                provider,
                Some(&format!(
                    "Provider '{}' requires '{}' but command not found. Using heuristic fallback.",
    ...
    if requires_api_key {
        let api_key_configured = match normalized_provider.as_str() {
            "codex" => {
            ...
            "openai" => {
                std::env::var("OPENAI_API_KEY").is_ok() || std::env::var("CODEX_API_KEY").is_ok()
            }
            _ => false,
        };
