...
const PROVIDER_COMMANDS: Partial<Record<string, string>> = {
  auggie: "auggie",
  claude: "claude",
  codex: "codex-acp",
...
async function getAuditProviderReadinessError(provider: string): Promise<string | null> {
  const normalizedProvider = provider.trim().toLowerCase();
  if (!normalizedProvider) {
    return "Provider is empty";
  }

  const command = PROVIDER_COMMANDS[normalizedProvider];
  if (command) {
    const resolved = await which(command);
    if (!resolved) {
      return `Provider '${normalizedProvider}' requires '${command}' but command not found. Using heuristic fallback.`;
    }
  }

  if ((normalizedProvider === "codex" || normalizedProvider === "openai") && !process.env.OPENAI_API_KEY && !process.env.CODEX_API_KEY) {
    return `Provider '${normalizedProvider}' API key not configured. Using heuristic fallback.`;
  }

  if ((normalizedProvider === "claude" || normalizedProvider === "anthropic") && !process.env.ANTHROPIC_API_KEY && !process.env.ANTHROPIC_AUTH_TOKEN) {
    return `Provider '${normalizedProvider}' API key not configured. Using heuristic fallback.`;
  }
...
async function executeAuditorCommand(repoRoot: string, workspaceId: string, source: string, provider: string) {
  const toolRepoRoot = getCurrentRoutaRepoRoot() ?? path.resolve(process.cwd());
  const localBinaryPath = path.join(toolRepoRoot, "target", "debug", "routa");
...
async function runInstructionAudit(
  ...
  const readinessError = await getAuditProviderReadinessError(provider);
  if (readinessError) {
    return buildHeuristicAudit(source, Date.now() - start, provider, readinessError);
  }
  try {
