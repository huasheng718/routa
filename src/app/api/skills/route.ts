...
import { localizeSkillDefinition, normalizeSkillLocale } from "@/core/skills/skill-localization";
...
export async function GET(request: NextRequest) {
  ...
  const locale = normalizeSkillLocale(
  ...
  if (name) {
    ...
    const localizedSkill = localizeSkillDefinition(skill, locale);
    return NextResponse.json({
      name: localizedSkill.name,
      description: localizedSkill.description,
      content: localizedSkill.content,
      license: localizedSkill.license,
      compatibility: localizedSkill.compatibility,
      metadata: localizedSkill.metadata,
    });
  }
  ...
  const allSkills = Array.from(allSkillsMap.values()).map((skill) => localizeSkillDefinition(skill, locale));
