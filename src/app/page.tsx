  ...
  const hasProviderConfig =
    hydrated
      ? hasSavedProviderConfiguration(loadDefaultProviders(), loadProviderConnections(), {
        ...
        customModelCount: loadModelDefinitions().filter((definition) =>
          Boolean(definition.alias?.trim() && definition.modelName?.trim() && (definition.baseUrl?.trim() || definition.apiKey?.trim()))
        ).length,
      })
      : false;
  const hasAvailableProviderRuntime =
    hydrated && acp.providers.some((provider) => provider.status === "available");
  const hasProviderReady = hasProviderConfig || hasAvailableProviderRuntime;
  const needsInlineOnboarding =
    hasWorkspace &&
    !onboardingCompleted &&
    (!hasProviderReady || preferredMode === null);
            ...
            {!hasWorkspace ? (
              <div className="flex min-h-0 flex-1 items-center justify-center p-6">
                <OnboardingCard
                  hasWorkspace={false}
                  workspaceTitle={null}
                  hasProviderConfig={hasProviderReady}
                  hasCodebase={false}
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4">
              ...
              <section className="mt-6 rounded-[24px] border border-black/6 bg-white/70 p-4 dark:border-white/8 dark:bg-white/5">
                ...
                <div className="mt-3 space-y-2 text-sm text-slate-600 dark:text-slate-300">
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <span>模型配置</span>
                    <span className={hasProviderReady ? "text-emerald-600 dark:text-emerald-300" : "text-amber-600 dark:text-amber-300"}>
                      {hasProviderReady ? "已就绪" : "待配置"}
                    </span>
                  </div>
                </div>
                <button
