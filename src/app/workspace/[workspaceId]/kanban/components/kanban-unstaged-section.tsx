...
export function KanbanUnstagedSection({
  ...
  const actions = (
    <>
      ...
      >
        ...
        {t.kanban.stageSelected.replace("{label}", selectedLabel)}
      </button>
      ...
      >
        ...
        {t.kanban.discardSelected.replace("{label}", selectedLabel)}
      </button>
      ...
      <label className="ml-auto flex items-center gap-1.5 text-[10px] text-slate-600 dark:text-slate-400">
        ...
        />
        <span className="opacity-50">{t.kanban.autoCommit}</span>
      </label>
    </>
  );
  ...
  return (
    <KanbanFileChangesSection
      title={t.kanban.unstagedTitle}
      subtitle={files.length > 0 ? t.kanban.filesWithChanges.replace("{count}", String(files.length)) : undefined}
      files={files}
      embedded={embedded}
      showCheckbox={true}
