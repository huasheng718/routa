...
import type { InputContext } from "@/client/components/tiptap-input";
...
import { ensureWorkspaceCodebase } from "@/client/utils/workspace-codebases";
...
import { resolveModelDisplayLabel } from "@/client/components/settings-panel-shared";
...
export function SessionPageClient() {
  ...
  useEffect(() => {
    ...
    setRepoSelection(loadRepoSelection("chat", workspaceId));
  }, [workspaceId]);

  // Auto-select default codebase as repo when workspace changes
  useEffect(() => {
    if (repoSelection?.path) return;
  ...
  }, [codebases, repoSelection?.path]);

  useEffect(() => {
    if (!workspaceId) return;
    saveRepoSelection("chat", workspaceId, repoSelection);
  }, [repoSelection, workspaceId]);

  useEffect(() => {
    void ensureWorkspaceCodebase(workspaceId, repoSelection, codebases, fetchCodebases).catch(() => {});
  }, [codebases, fetchCodebases, repoSelection, workspaceId]);
  ...
  const defaultChatModelLabel = resolveModelDisplayLabel(resolveAgentConfig(selectedAgent).model);
  ...
  const handleRecoverSession = useCallback(async (text: string, context: InputContext): Promise<string | null> => {
    await ensureConnected();
    ...
    const {
      ...
      baseUrl,
      apiKey,
    } = resolveAgentConfig(role, context.provider, context.model, selectedSpec);
    ...
    router.push(buildSessionHref(result.sessionId));
    return result.sessionId;
  }, [
    acp,
    buildSessionHref,
      ...
      <div className="flex-1 flex min-h-0 relative">
        ...
        <main className="flex flex-1 min-w-0 flex-col">
          ...
          <ChatPanel
            ...
            codebases={codebases}
            defaultModelLabel={defaultChatModelLabel}
