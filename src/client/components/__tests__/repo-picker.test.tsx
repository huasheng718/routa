...
describe("RepoPicker", () => {
  ...
  it("loads a local repository from the local project tab", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /choose local folder/i }));
  ...
  it("switches from search to local mode when the query looks like a file path", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText("~/code/routa-js")).toBeTruthy();
      expect(screen.getByRole("button", { name: /use local project/i }).hasAttribute("disabled")).toBe(false);
    });
  });
