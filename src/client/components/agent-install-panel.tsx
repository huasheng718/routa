...
function isMissingTauriInvokeError(err: unknown): boolean {
  return err instanceof Error && err.message.includes("Tauri invoke not available");
}
...
const INSTALL_REQUEST_TIMEOUT_MS = 120_000;
...
export function AgentInstallPanel({ embedded = false }: AgentInstallPanelProps) {
  ...
  const fetchAgentsViaWeb = useCallback(async (refresh = false) => {
  ...
  const fetchAgents = useCallback(
      ...
      try {
        ...
        if (isTauri.current) {
          try {
            ...
            if (!isMissingTauriInvokeError(err)) throw err;
            isTauri.current = false;
            await fetchAgentsViaWeb(refresh);
          }
        } else {
          await fetchAgentsViaWeb(refresh);
        }
    ...
    [convertTauriRegistry, fetchAgentsViaWeb, t.agents.failedToLoad]
  );
    ...
    async (agentId: string, _distType?: string) => {
      ...
      try {
        if (isTauri.current) {
          ...
          const timeout = setTimeout(() => controller.abort(), INSTALL_REQUEST_TIMEOUT_MS);
          const res = await desktopAwareFetch("/api/acp/install", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ agentId, distributionType: _distType }),
        ...
        if (err instanceof Error && err.name === "AbortError") {
          setError("安装超时：下载或预热时间过长。请检查网络，或优先安装支持 npx/uvx 的 Agent。");
        } else {
