...
interface ChatPanelProps {
  ...
  onRecoverSession?: (text: string, context: InputContext) => Promise<string | null>;
  ...
  /** Read-only label for the effective default model shown by the input */
  defaultModelLabel?: string;
}

function isRecoverableDetachedSessionError(error: unknown): boolean {
  const message = error instanceof Error ? error.message : String(error ?? "");
  const normalized = message.toLowerCase();
  return (
    normalized.includes("session is currently owned by instance")
    || normalized.includes("session ownership lease expired")
    || normalized.includes("cannot be resumed on a different instance")
    || normalized.includes("no acp agent process for session")
    || normalized.includes("acp agent (")
...
export function ChatPanel({
  ...
  const handleSend = useCallback(async (text: string, context: InputContext) => {
    ...
    setMessagesBySession((prev) => {
      ...
      arr.push({ id: userMessageId, role: "user", content: prefix + text, timestamp: new Date() });
      next[sid] = arr;
      return next;
    });
    ...
    try {
      ...
      if (onRecoverSession && isRecoverableDetachedSessionError(err)) {
        setMessagesBySession((prev) => {
          const next = { ...prev };
          next[sid] = (next[sid] ?? []).filter((message) => message.id !== userMessageId);
          return next;
        });
        await onRecoverSession(text, {
          ...context,
          cwd,
        });
      }
  ...
  }, [activeSessionId, onEnsureSession, onRecoverSession, onSelectSession, promptSession, repoSelection, onLoadSkill, acp, resetStreamingRefs, setMessagesBySession]);
