...
import { loadRepoSelection, saveRepoSelection } from "../utils/repo-selection-storage";
...
import { resolveModelDisplayLabel } from "./settings-panel-shared";
import { desktopAwareFetch } from "../utils/diagnostics";
import { useTranslation } from "@/i18n";
import { normalizeSpecialistLocale } from "@/core/specialists/specialist-locale";
...
export function HomeInput({
  ...
  const { codebases, fetchCodebases } = useCodebases(selectedWorkspaceId ?? "");
  ...
  // Load specialists
  useEffect(() => {
    const specialistLocale = normalizeSpecialistLocale(locale);
    desktopAwareFetch(`/api/specialists?locale=${encodeURIComponent(specialistLocale)}`)
  ...
  const ensureAcpConnected = useCallback(async () => {
  ...
  useEffect(() => {
    ...
    const savedSelection = loadRepoSelection("chat", selectedWorkspaceId);
    repoSelectionRef.current = savedSelection;
    setRepoSelection(savedSelection);
  }, [selectedWorkspaceId]);

  useEffect(() => {
    if (!selectedWorkspaceId) return;
    saveRepoSelection("chat", selectedWorkspaceId, repoSelection);
  }, [repoSelection, selectedWorkspaceId]);

  useEffect(() => {
    void ensureWorkspaceCodebase(selectedWorkspaceId, repoSelection, codebases, fetchCodebases).catch(() => {});
  }, [codebases, fetchCodebases, repoSelection, selectedWorkspaceId]);

  // Auto-select default codebase when no explicit chat repo was saved
  useEffect(() => {
    if (repoSelectionRef.current?.path) return;
  ...
  const handleSend = useCallback(
    ...
    [acp, buildSessionUrl, ensureAcpConnected, lockedSpecialistId, requireRepoSelection, selectedRole, selectedWorkspaceId, selectedSpecialistId, router, onSessionCreated, specialists],
  );
  ...
  const defaultModelLabel = resolveModelDisplayLabel(selectedSpecialist?.model)
    ?? resolveModelDisplayLabel(loadProviderConnectionConfig(acp.selectedProvider).model);
      ...
      <div className="group relative" id="home-input-container">
        ...
        <div className={shellClass}>
          ...
          <TiptapInput
            onSend={handleSend}
            onEditorFocus={() => {
              void ensureAcpConnected();
            }}
            placeholder={t.home.inputPlaceholder}
            disabled={isSubmitting || (requireRepoSelection && !repoSelection?.path)}
