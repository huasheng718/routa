...
const LEGACY_OPENCODE_MODEL = "opencode/qwen3.6-plus-free";
...
function loadModelDefinitionsFromStorage(): ModelDefinition[] {
  try {
...
function normalizeStoredRoleConfig(value: unknown): AgentModelConfig | undefined {
  if (!value) return undefined;
  return typeof value === "string" ? { provider: value } : (value as AgentModelConfig);
}

function isLegacyOpencodeSetting(value: AgentModelConfig | undefined): boolean {
  if (!value) return true;
  const provider = value.provider ?? "opencode";
  return provider === "opencode" && (!value.model || value.model === LEGACY_OPENCODE_MODEL);
}

function maybePromoteSingleCustomModelAlias(): void {
  if (typeof window === "undefined") return;
  try {
    const defs = loadModelDefinitionsFromStorage().filter((definition) =>
      Boolean(definition.alias?.trim() && definition.modelName?.trim() && (definition.baseUrl?.trim() || definition.apiKey?.trim()))
    );
    if (defs.length !== 1) return;

    const alias = defs[0]!.alias.trim();

    const rawDefaults = localStorage.getItem(STORAGE_KEY);
    const parsedDefaults = rawDefaults ? (JSON.parse(rawDefaults) as Record<string, unknown>) : {};
    const roleConfigs = Object.fromEntries(
      AGENT_ROLES.map((role) => [role, normalizeStoredRoleConfig(parsedDefaults[role])]),
    ) as DefaultProviderSettings;
    const shouldPromoteRoles = AGENT_ROLES.every((role) => isLegacyOpencodeSetting(roleConfigs[role]));

    const rawConnections = localStorage.getItem(CONNECTIONS_STORAGE_KEY);
    const parsedConnections = rawConnections ? (JSON.parse(rawConnections) as ProviderConnectionsStorage) : {};
    const opencodeConnection = parsedConnections.opencode ?? {};
    const shouldPromoteConnection = !opencodeConnection.model || opencodeConnection.model === LEGACY_OPENCODE_MODEL;

    if (!shouldPromoteRoles || !shouldPromoteConnection) return;

    const nextRoleConfig: AgentModelConfig = { provider: "opencode", model: alias };
    const nextDefaults: DefaultProviderSettings = {
      ...roleConfigs,
      ROUTA: nextRoleConfig,
      CRAFTER: nextRoleConfig,
      GATE: nextRoleConfig,
      DEVELOPER: nextRoleConfig,
    };
    const nextConnections: ProviderConnectionsStorage = {
      ...parsedConnections,
      opencode: {
        ...opencodeConnection,
        model: alias,
    ...
    localStorage.setItem(STORAGE_KEY, JSON.stringify(nextDefaults));
    localStorage.setItem(CONNECTIONS_STORAGE_KEY, JSON.stringify(nextConnections));
  } catch {
    // Ignore migration errors and keep existing settings.
  }
}

export function loadDefaultProviders(): DefaultProviderSettings {
  if (typeof window === "undefined") return {};
  maybePromoteSingleCustomModelAlias();
  try {
...
export function resolveModelDisplayLabel(modelAliasOrName?: string): string | undefined {
  if (!modelAliasOrName) return undefined;
  const definition = getModelDefinitionByAlias(modelAliasOrName);
  const effectiveModel = definition?.modelName ?? modelAliasOrName;
