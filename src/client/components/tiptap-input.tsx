...
import { loadProviderConnectionConfig, resolveModelDisplayLabel } from "./settings-panel-shared";
...
interface TiptapInputProps {
  ...
  /** Read-only label for the effective default model when no temporary override is selected */
...
export function TiptapInput({
  ...
  const fallbackDefaultModelLabel = useMemo(() => {
    if (defaultModelLabel) return defaultModelLabel;
    return resolveModelDisplayLabel(loadProviderConnectionConfig(selectedProvider).model);
  }, [defaultModelLabel, selectedProvider]);
            ...
            <div ref={modelDropdownRef}>
              ...
              >
                ...
                <span className={`font-medium truncate ${isHero ? "max-w-[180px] text-slate-700 dark:text-slate-200" : "max-w-[140px] text-slate-700 dark:text-slate-300"}`}>
                  {selectedModel ? selectedModel.split("/").pop() : (fallbackDefaultModelLabel ?? t.chatPanel.defaultModel)}
                </span>
                {modelLoading
