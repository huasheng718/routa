...
const BINARY_DOWNLOAD_TIMEOUT_MS = 90_000;
...
export async function downloadBinary(
  ...
  try {
    ...
    const timeout = setTimeout(() => controller.abort(), BINARY_DOWNLOAD_TIMEOUT_MS);
    ...
    try {
      ...
      if (error instanceof Error && error.name === "AbortError") {
        throw new Error(
          `Binary download timed out after ${Math.round(BINARY_DOWNLOAD_TIMEOUT_MS / 1000)}s. ` +
          "Please check your network/proxy, or try an npx/uvx-based agent instead."
        );
      }
      throw error;
    } finally {
