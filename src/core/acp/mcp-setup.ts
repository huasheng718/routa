...
export async function ensureMcpForProvider(
  ...
  switch (baseId) {
    case "opencode":
      return await ensureMcpForOpenCode(mcpEndpoint, cfg.workspaceId, customServers, model, connection);
    case "auggie":
...
async function ensureMcpForOpenCode(
  ...
  try {
    ...
    const trimmedApiKey = connection?.apiKey?.trim();
    if (trimmedModel && trimmedBaseUrl && trimmedApiKey) {
      const providerId = `routa-${trimmedModel.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "custom-model"}`;
      const providers = (existing.provider ?? {}) as Record<string, unknown>;
      const currentProvider = (providers[providerId] ?? {}) as Record<string, unknown>;
      const currentModels = (currentProvider.models ?? {}) as Record<string, unknown>;

      currentModels[trimmedModel] = {
        name: trimmedModel,
      };

      providers[providerId] = {
        ...currentProvider,
        name: (typeof currentProvider.name === "string" && currentProvider.name.trim()) || `Routa ${trimmedModel}`,
        npm: "@ai-sdk/openai",
      ...
      existing.model = `${providerId}/${trimmedModel}`;
    } else if (trimmedModel) {
      existing.model = trimmedModel;
    }
