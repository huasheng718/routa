...
function buildEmptySurfaceSignals(surface: HarnessSignalsMode): HarnessSurfaceSignals {
  const capitalized = surface[0]?.toUpperCase() + surface.slice(1);
  return {
    configPath: path.join("docs", "harness", `${surface}.yml`),
    title: `${capitalized} Signals`,
    summary: `No docs/harness/${surface}.yml found for the selected repository.`,
    overviewRows: [],
    entrypointGroups: [],
  };
}
...
async function detectSurfaceSignals(
  ...
  const absoluteConfigPath = joinRepoPath(repoRoot, configPath);
  if (!fs.existsSync(absoluteConfigPath)) {
    warnings.push(`Missing ${configPath} for repository: ${repoRoot}`);
    return buildEmptySurfaceSignals(surface);
  }
