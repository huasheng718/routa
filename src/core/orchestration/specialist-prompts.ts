...
import { DEFAULT_SPECIALIST_LOCALE, normalizeSpecialistLocale } from "../specialists/specialist-locale";
...
export async function loadSpecialists(locale: string = DEFAULT_SPECIALIST_LOCALE): Promise<SpecialistConfig[]> {
  ...
  const cached = _cachedSpecialists.get(normalizedLocale);
  if (cached) return cached;

  if (_useDatabase) {
    // Use the new database-aware loader
    const specialists = await loadSpecialistsFromAllSources(normalizedLocale);
    _cachedSpecialists.set(normalizedLocale, specialists);
    return specialists;
  } else {
    // Use the original file-based loader
    try {
      const fromFiles = loadAllSpecialists(normalizedLocale);
    ...
    _cachedSpecialists.set(normalizedLocale, [...HARDCODED_SPECIALISTS]);
  }

  return _cachedSpecialists.get(normalizedLocale) ?? [...HARDCODED_SPECIALISTS];
}
...
export async function reloadSpecialists(locale: string = DEFAULT_SPECIALIST_LOCALE): Promise<SpecialistConfig[]> {
  ...
  return loadSpecialists(normalizeSpecialistLocale(locale));
}
