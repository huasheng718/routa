...
import { DEFAULT_SPECIALIST_LOCALE, normalizeSpecialistLocale } from "./specialist-locale";
...
export async function loadSpecialistsFromAllSources(
  locale: string = DEFAULT_SPECIALIST_LOCALE,
  ...
  try {
    ...
    const userFiles = loadUserSpecialists(normalizedLocale);
    ...
    const bundledFiles = loadBundledSpecialists(normalizedLocale);
    ...
    _cachedSpecialists.set(normalizedLocale, result);
    console.log(
...
export async function reloadSpecialistsFromAllSources(
  ...
  _cachedSpecialists = new Map();
  return loadSpecialistsFromAllSources(locale, specialistStore);
}
